...
int main() {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "        SQ-SIMD Experiment            " << std::endl;
    std::cout << "========================================" << std::endl;
    ...
    // Train SQ quantizer
    SQQuantizer sq(dim);
    sq.train(base.data(), base_num);
    auto base_codes = sq.encode_batch(base.data(), base_num);

    std::cout << "SQ trained. Compression ratio: 4x (32bit -> 8bit)" << std::endl;
    std::cout << "----------------------------------------" << std::endl;

    // Ground truth: serial flat search on original vectors
    ...
    std::cout << "ADC Strategy (Asymmetric Distance Computation)" << std::endl;
    ...
    for (int p : p_values) {
        ...
        for (int q = 0; q < query_num; ++q) {
            results[q] = sq_search(base.data(), base_codes,
                                   queries.data() + q * dim,
                                   base_num, dim, k, p, sq, true);
        }
        auto t2 = std::chrono::high_resolution_clock::now();
        double time_sq = std::chrono::duration<double, std::milli>(t2 - t1).count();
        ...
        std::cout << std::setw(8) << p
                  << std::setw(15) << std::fixed << std::setprecision(3) << (time_sq / query_num)
                  << std::setw(12) << std::fixed << std::setprecision(4) << avg_recall
                  << std::setw(12) << std::fixed << std::setprecision(2) << (time_serial / time_sq)
                  << std::endl;
    }
