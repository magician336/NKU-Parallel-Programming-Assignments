...
public:
    ...
    // 构建LUT：query到每个子空间所有中心的IP距离表
    // lut[sub * ksub_ + c] = 1 - dot(query_sub, centroid_c)
        ...
        for (int sub = 0; sub < m_; ++sub) {
            ...
            for (int c = 0; c < ksub_; ++c) {
                ...
                for (; d + 4 <= dsub_; d += 4) {
                    ...
                    sum = fmadd(q, cn, sum);  // dot product
                }
                float dist = reduce_sum(sum);
                for (; d < dsub_; ++d) {
                    dist += sub_query[d] * cent[d];
                }
                lut[sub * ksub_ + c] = 1.0f - dist;  // IP distance
#else
                float dist = 0.0f;
                for (size_t d = 0; d < dsub_; ++d) {
                    dist += sub_query[d] * cent[d];
                }
                lut[sub * ksub_ + c] = 1.0f - dist;
#endif
            }
        }
        return lut;
    ...
    // ADC距离：sum over subspaces of LUT[sub][code[sub]]
    // LUT里已经存了子空间的IP距离，直接累加即可
