...
void run_hnsw_hnsw_mpi_evaluation(int thread_count, int nprobe,
                                  HNSWHNSWSearcher &searcher,
                                  ...
                                  int local_base_number, int rank, int size)
{
    ...
    if (thread_count == 1)
    {
        ...
        {
            auto pq = searcher.search(test_query + q * vecdim, k, nprobe);
            int idx = 0;
            while (!pq.empty() && idx < k)
            {
                Candidate c = pq.top();
                pq.pop();
                local_results[q * k + idx] = {c.dist,
    ...
    else
    {
        ...
        std::vector<std::vector<Candidate>> thread_results(test_number);

        tp::parallel_for_static(0, test_number, [&](size_t q)
                                {
            thread_results[q] = std::vector<Candidate>();
            auto pq = searcher.search(test_query + q * vecdim, k, nprobe);
            while (!pq.empty()) {
                thread_results[q].push_back(pq.top());
                pq.pop();
            } });
        ...
        {
            int idx = 0;
            for (const auto &c : thread_results[q])
            {
                if (idx >= k)
                    break;
                local_results[q * k + idx] = {c.dist,
...
int main(int argc, char *argv[])
{
    ...
    std::vector<std::vector<std::pair<float, size_t>>> gt;
    ...
    HNSWHNSWSearcher searcher(&index);

    std::ofstream csv_file;
    if (rank == 0)
    {
        mkdir("files", 0777);
        csv_file.open("files/hnsw_hnsw_mpi_results.csv");
        csv_file << "Method,Threads,NProbe,Recall,Latency_us,QPS,ImbalanceRatio,MPIOverhead_us\n";

        std::cerr << "\n=== HNSW-on-HNSW MPI Evaluation ===\n";
    ...
    for (int t : thread_configs)
    {
        ...
        {
            run_hnsw_hnsw_mpi_evaluation(t, probe, searcher, test_query, gt,
                                         test_number, vecdim, k, csv_file,
                                         "HNSW-HNSW-MPI", local_base_number, rank, size);
        }
    }
    ...
    {
        csv_file.close();
        std::cerr << "\n文件保存至: files/hnsw_hnsw_mpi_results.csv\n";
    }
