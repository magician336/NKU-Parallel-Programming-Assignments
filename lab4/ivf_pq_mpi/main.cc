    ...
    tp::parallel_region([&](int tid) {
        ...
        for (size_t i = start_idx; i < end_idx; ++i) {
            ...
            while (!pq_res.empty() && idx >= 0) {
                ...
                DistancePair dp;
                dp.dist = c.dist;
                dp.id = rank * local_base_number + static_cast<int>(c.id);
...
int main(int argc, char* argv[]) {
    ...
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            run_mpi_evaluation(t, probe, &adc_searcher, test_query, test_gt,
                               ...
                               "ADC-MPI", local_base_number, rank, size);
        }
    }
    ...
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            run_mpi_evaluation(t, probe, &sdc_searcher, test_query, test_gt,
                               test_number, vecdim, test_gt_d, k, csv_file,
                               "SDC-MPI", local_base_number, rank, size);
        }
    }
    ...
    SIMDSearcher simd_searcher(&index, local_base);
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            run_mpi_evaluation(t, probe, &simd_searcher, test_query, test_gt,
                               test_number, vecdim, test_gt_d, k, csv_file,
                               "SIMD-MPI", local_base_number, rank, size);
        }
    }

    if (rank == 0) {
        csv_file.close();
        std::cerr << "\nResults saved to files/ivfpq_mpi_results.csv\n";
        delete[] test_gt;
    }
