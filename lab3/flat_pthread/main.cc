...
int main() {
    ...
    for (int nt : thread_counts) {
        ...
        auto results = flat_search_pthread_query(base, queries, base_num, dim,
                                                  query_num, k, nt, dtype);
    ...
    for (int nt : thread_counts) {
        t1 = std::chrono::high_resolution_clock::now();
        auto results = flat_search_pthread_partition(base, queries, base_num, dim,
                                                      query_num, k, 2 * k, nt, dtype);
        t2 = std::chrono::high_resolution_clock::now();
        double time_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();
