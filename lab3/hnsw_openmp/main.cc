...
// Layer-0随机入口并行(OpenMP)

std::vector<std::pair<float, size_t>> hnsw_search_layer0_random_parallel_openmp(
    ...
    int num_threads)
{
    ...
    #pragma omp parallel
    {
        std::vector<std::pair<dist_t, tableint>> local;
        #pragma omp for schedule(dynamic)
        for (int i = 0; i < actual_threads; ++i) {
            auto pq = index.searchBaseLayerST<true>(entry_points[i], query, std::max(ef, k));
            while (!pq.empty()) {
                local.push_back({pq.top().first, pq.top().second});
                pq.pop();
            }
        }
        #pragma omp critical
    ...
    all_candidates.erase(std::unique(all_candidates.begin(), all_candidates.end(),
        [](const std::pair<dist_t, tableint>& a, const std::pair<dist_t, tableint>& b) { return a.second == b.second; }), all_candidates.end());

    std::vector<std::pair<float, size_t>> result;
    result.reserve(k);
    for (size_t i = 0; i < all_candidates.size() && (int)result.size() < k; ++i) {
        result.push_back({all_candidates[i].first, (size_t)all_candidates[i].second});
    }
    return result;
}
    ...
    for (int ef : ef_configs) {
        ...
        for (int nt : thread_counts) {
            ...
            if (strategy_name == "Batch-level Parallel") {
                ...
                for (int q = 0; q < (int)query_num; ++q) {
                    results[q] = hnsw_search_layer0_random_parallel_openmp(index, queries + q * dim, k, ef, nt);
                }
            }
