#ifndef SIMD_UTILS_H
#define SIMD_UTILS_H

#include <cstddef>
#include <cstdint>

#if defined(__ARM_NEON) || defined(__aarch64__)
  #include <arm_neon.h>
  #define USE_NEON
#elif defined(__SSE__)
  #include <xmmintrin.h>
  #include <emmintrin.h>
  #ifdef __AVX__
    #include <immintrin.h>
    #define USE_AVX
  #endif
  #ifdef __AVX2__
    #define USE_AVX2
  #endif
  #define USE_SSE
#endif

// 128bit floatx4
struct simd4f {
#if defined(USE_NEON)
    float32x4_t v;
#elif defined(USE_SSE)
    __m128 v;
#else
    float v[4];
#endif

    simd4f() = default;

#if defined(USE_NEON)
    explicit simd4f(float32x4_t _v) : v(_v) {}
    explicit simd4f(float f) : v(vmovq_n_f32(f)) {}
    explicit simd4f(float a, float b, float c, float d) {
        float data[4] = {a, b, c, d};
        v = vld1q_f32(data);
    }
#elif defined(USE_SSE)
    explicit simd4f(__m128 _v) : v(_v) {}
    explicit simd4f(float f) : v(_mm_set1_ps(f)) {}
    explicit simd4f(float a, float b, float c, float d)
        : v(_mm_setr_ps(a, b, c, d)) {}
#else
    explicit simd4f(float f) { v[0]=v[1]=v[2]=v[3]=f; }
    explicit simd4f(float a, float b, float c, float d) {
        v[0]=a; v[1]=b; v[2]=c; v[3]=d;
    }
#endif

    static simd4f load(const float* p) {
    #if defined(USE_NEON)
        return simd4f(vld1q_f32(p));
    #elif defined(USE_SSE)
        return simd4f(_mm_load_ps(p));
    #else
        simd4f r; r.v[0]=p[0]; r.v[1]=p[1]; r.v[2]=p[2]; r.v[3]=p[3]; return r;
    #endif
    }

    static simd4f loadu(const float* p) {
    #if defined(USE_NEON)
        return simd4f(vld1q_f32(p));
    #elif defined(USE_SSE)
        return simd4f(_mm_loadu_ps(p));
    #else
        simd4f r; r.v[0]=p[0]; r.v[1]=p[1]; r.v[2]=p[2]; r.v[3]=p[3]; return r;
    #endif
    }

    void store(float* p) const {
    #if defined(USE_NEON)
        vst1q_f32(p, v);
    #elif defined(USE_SSE)
        _mm_store_ps(p, v);
    #else
        p[0]=v[0]; p[1]=v[1]; p[2]=v[2]; p[3]=v[3];
    #endif
    }

    void storeu(float* p) const {
    #if defined(USE_NEON)
        vst1q_f32(p, v);
    #elif defined(USE_SSE)
        _mm_storeu_ps(p, v);
    #else
        p[0]=v[0]; p[1]=v[1]; p[2]=v[2]; p[3]=v[3];
    #endif
    }

    // gather 4 floats
    static simd4f gather(const float* base, const int* indices) {
    #if defined(USE_AVX2)
        __m128i idx = _mm_loadu_si128((const __m128i*)indices);
        return simd4f(_mm_i32gather_ps(base, idx, 4));
    #else
        return simd4f(base[indices[0]], base[indices[1]], base[indices[2]], base[indices[3]]);
    #endif
    }
};

inline simd4f operator+(simd4f a, simd4f b) {
#if defined(USE_NEON)
    return simd4f(vaddq_f32(a.v, b.v));
#elif defined(USE_SSE)
    return simd4f(_mm_add_ps(a.v, b.v));
#else
    simd4f r; for(int i=0;i<4;i++) r.v[i]=a.v[i]+b.v[i]; return r;
#endif
}

inline simd4f operator-(simd4f a, simd4f b) {
#if defined(USE_NEON)
    return simd4f(vsubq_f32(a.v, b.v));
#elif defined(USE_SSE)
    return simd4f(_mm_sub_ps(a.v, b.v));
#else
    simd4f r; for(int i=0;i<4;i++) r.v[i]=a.v[i]-b.v[i]; return r;
#endif
}

inline simd4f operator*(simd4f a, simd4f b) {
#if defined(USE_NEON)
    return simd4f(vmulq_f32(a.v, b.v));
#elif defined(USE_SSE)
    return simd4f(_mm_mul_ps(a.v, b.v));
#else
    simd4f r; for(int i=0;i<4;i++) r.v[i]=a.v[i]*b.v[i]; return r;
#endif
}

inline simd4f fmadd(simd4f a, simd4f b, simd4f c) {
#if defined(USE_NEON) && defined(__aarch64__)
    return simd4f(vfmaq_f32(c.v, a.v, b.v));
#elif defined(USE_AVX2)
    return simd4f(_mm_fmadd_ps(a.v, b.v, c.v));
#elif defined(USE_SSE)
    return simd4f(_mm_add_ps(c.v, _mm_mul_ps(a.v, b.v)));
#else
    simd4f r; for(int i=0;i<4;i++) r.v[i]=c.v[i]+a.v[i]*b.v[i]; return r;
#endif
}

inline float reduce_sum(simd4f a) {
#if defined(USE_NEON)
    float32x2_t low  = vget_low_f32(a.v);
    float32x2_t high = vget_high_f32(a.v);
    low = vpadd_f32(low, high);
    low = vpadd_f32(low, low);
    return vget_lane_f32(low, 0);
#elif defined(USE_SSE)
    __m128 shuf = _mm_shuffle_ps(a.v, a.v, _MM_SHUFFLE(1, 0, 3, 2));
    __m128 sums = _mm_add_ps(a.v, shuf);
    shuf = _mm_shuffle_ps(sums, sums, _MM_SHUFFLE(2, 3, 0, 1));
    sums = _mm_add_ps(sums, shuf);
    return _mm_cvtss_f32(sums);
#else
    return a.v[0] + a.v[1] + a.v[2] + a.v[3];
#endif
}

// 128bit uint8x16
struct simd16u8 {
#if defined(USE_NEON)
    uint8x16_t v;
#elif defined(USE_SSE)
    __m128i v;
#else
    uint8_t v[16];
#endif

    simd16u8() = default;

#if defined(USE_NEON)
    explicit simd16u8(uint8x16_t _v) : v(_v) {}
    explicit simd16u8(uint8_t u) : v(vmovq_n_u8(u)) {}
#elif defined(USE_SSE)
    explicit simd16u8(__m128i _v) : v(_v) {}
    explicit simd16u8(uint8_t u) : v(_mm_set1_epi8((char)u)) {}
#else
    explicit simd16u8(uint8_t u) { for(int i=0;i<16;i++) v[i]=u; }
#endif

    static simd16u8 loadu(const uint8_t* p) {
    #if defined(USE_NEON)
        return simd16u8(vld1q_u8(p));
    #elif defined(USE_SSE)
        return simd16u8(_mm_loadu_si128((__m128i const*)p));
    #else
        simd16u8 r; for(int i=0;i<16;i++) r.v[i]=p[i]; return r;
    #endif
    }
};

#endif // SIMD_UTILS_H
