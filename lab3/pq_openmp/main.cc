...
int main() {
    ...
    for (int nt : thread_counts) {
        t1 = std::chrono::high_resolution_clock::now();
        auto results = pq_search_openmp_query(base, base_codes, queries,
    ...
    for (int nt : thread_counts) {
        t1 = std::chrono::high_resolution_clock::now();
        auto results = pq_search_openmp_partition(base, base_codes, queries,
                                                   base_num, dim, query_num, k, p, nt, pq);
        t2 = std::chrono::high_resolution_clock::now();
        double time_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();
    ...
    for (int nt : thread_counts) {
        t1 = std::chrono::high_resolution_clock::now();
        auto results = pq_search_openmp_batch(base, base_codes, queries,
                                               base_num, dim, query_num, k, p, nt, pq);
        t2 = std::chrono::high_resolution_clock::now();
        double time_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();
