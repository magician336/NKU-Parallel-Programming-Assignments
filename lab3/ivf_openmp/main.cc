...
void run_evaluation_openmp(int thread_count, int nprobe, BaseSearcher* searcher,
    ...
    if (thread_count == 8) {
        std::string profile_file = "files/ivfpq_profile_omp_t" + std::to_string(thread_count)
...
int main(int argc, char *argv[]) {
    ...
    std::ofstream csv_file("files/ivfpq_tradeoff_omp.csv");
    ...
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            std::cerr << "\n>>> Running ADC OpenMP: Threads=" << t << ", NProbe=" << probe << "\n";
            float tmp_r = 0.0f, tmp_l = 0.0f;
            MicroProfiler::reset();
            run_evaluation_openmp(t, probe, &adc_searcher, test_query, test_gt_data,
                                  test_number, vecdim, test_gt_d, k, csv_file, "ADC-OMP",
                                  &tmp_r, &tmp_l);
            if (t == 8 && probe == 128) {
                final_recall = tmp_r;
    ...
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            std::cerr << "\n>>> Running SDC OpenMP: Threads=" << t << ", NProbe=" << probe << "\n";
            MicroProfiler::reset();
            run_evaluation_openmp(t, probe, &sdc_searcher, test_query, test_gt_data,
                                  test_number, vecdim, test_gt_d, k, csv_file, "SDC-OMP");
        }
    }
    ...
    for (int t : thread_configs) {
        for (int probe : nprobe_configs) {
            std::cerr << "\n>>> Running IVF-SIMD OpenMP: Threads=" << t << ", NProbe=" << probe << "\n";
            MicroProfiler::reset();
            run_evaluation_openmp(t, probe, &simd_searcher, test_query, test_gt_data,
                                  test_number, vecdim, test_gt_d, k, csv_file, "SIMD-OMP");
        }
    }
    ...
    std::cerr << "\nAll evaluations completed. Results in files/ivfpq_tradeoff_omp.csv\n";
    return 0;
}
